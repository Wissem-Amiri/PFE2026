...
export default function AdminDashboardPage() {
  ...
  const activities = [
    ...users.filter(u => u.role !== 'admin').map(u => ({ 
      ...
      type: 'registration', 
      user: u, 
      details: 'Requested Registration',
      status: u.status 
    })),
    ...leaves.map(l => ({ 
      id: l.id, 
  ...
  const stats = [
    { 
      ...
      count: users.filter(u => u.role !== 'admin').length.toString().padStart(2, '0'), 
      icon: <UserOutlined />, 
      color: '#F5F3FF', 
      iconColor: '#7C3AED',
      sub: `${users.filter(u => u.status === 'pending').length} pending approval`
    },
    { 
    ...
    { 
      ...
      count: candidatures.length.toString().padStart(2, '0'), 
      icon: <SolutionOutlined />, 
      color: '#EFF6FF', 
      iconColor: '#3B82F6',
      sub: 'Active applicants'
    },
  ]
