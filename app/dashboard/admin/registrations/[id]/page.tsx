    ...
    <div className="flex flex-col gap-[48px] pt-[24px]">
      ...
      <div className="flex flex-col gap-[8px]">
        ...
        <p className="text-[14px] text-[#475467] m-0">
          {user.candidate?.position ? `I specialise in ${user.candidate.position}.` : 'I specialize in UX/UI design, brand strategy, and Webflow development.'}
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-[64px]">
            ...
            {user.candidate?.bio ? (
              <div className="flex flex-col gap-[12px]">
                <p className="m-0 whitespace-pre-wrap">
                  {isBioExpanded || user.candidate.bio.length <= 250
                    ? user.candidate.bio
                    : `${user.candidate.bio.substring(0, 250)}...`}
                </p>
                {user.candidate.bio.length > 250 && (
                  <button
        ...
        <div className="w-[320px] flex flex-wrap gap-y-[24px]">
           ...
           {user.candidate?.website && (
             <div className="w-1/2 flex flex-col gap-[8px]">
                <span className="text-[12px] font-medium text-[#667085]">Website</span>
                <a href={user.candidate.website} target="_blank" rel="noreferrer" className="text-[14px] font-medium text-[#7F56D9] hover:underline flex items-center gap-[6px]">
                  <LinkOutlined /> {new URL(user.candidate.website).hostname}
                </a>
             </div>
           )}
           {user.candidate?.portfolio && (
             <div className="w-1/2 flex flex-col gap-[8px]">
                <span className="text-[12px] font-medium text-[#667085]">Portfolio</span>
                <a href={user.candidate.portfolio} target="_blank" rel="noreferrer" className="text-[14px] font-medium text-[#7F56D9] hover:underline flex items-center gap-[6px]">
      ...
      <div className="flex flex-col gap-[16px]">
        ...
        <div className="flex flex-col md:flex-row gap-[24px]">
           ...
           <div className="flex-1 flex flex-col gap-[12px]">
              ...
              {user.candidate?.motivational_letter_url ? (
                <div className="p-[16px] rounded-[12px] border border-[#EAECF0] bg-white flex items-center justify-between hover:border-[#7F56D9] transition-colors shadow-sm">
                  ...
                  <a href={user.candidate.motivational_letter_url} target="_blank" rel="noreferrer" className="text-[#7F56D9] font-medium text-[14px] hover:underline">View file</a>
                </div>
              ) : (
           ...
           <div className="flex-1 flex flex-col gap-[12px]">
              ...
              {user.candidate?.resume_url ? (
                <div className="p-[16px] rounded-[12px] border border-[#EAECF0] bg-white flex items-center justify-between hover:border-[#7F56D9] transition-colors shadow-sm">
                  ...
                  <a href={user.candidate.resume_url} target="_blank" rel="noreferrer" className="text-[#7F56D9] font-medium text-[14px] hover:underline">View file</a>
                </div>
              ) : (
      ...
      <div className="flex flex-col gap-[16px]">
        ...
        {(!user.candidate?.experiences || user.candidate.experiences.length === 0) ? (
          ...
          <div className="flex flex-wrap gap-[16px]">
            {user.candidate.experiences.map((exp: any, i: number) => (
    ...
    <div className="flex-1 p-[32px] h-full overflow-y-auto bg-white font-['Inter',sans-serif]">
      ...
      <div className="flex justify-between items-center mb-[48px]">
        ...
        <div className="flex gap-[12px]">
           <button 
             onClick={() => user.candidate?.resume_url && window.open(user.candidate.resume_url)}
